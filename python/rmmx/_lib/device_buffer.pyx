...
from rmmx._cuda.stream cimport Stream

from rmmx._cuda.stream import DEFAULT_STREAM

include '../_cuda/_runtime_extern.pxi'

from rmmx._lib.memory_resource cimport get_current_device_resource
...
cdef class DeviceBuffer:
    ...
    def copy(self):
        ...
        --------
        >>> import rmmx
        >>> db = rmmx.DeviceBuffer.to_device(b"abc")
        >>> db_copy = db.copy()
        >>> db.copy_to_host()
    ...
    cpdef copy_from_device(self, cuda_ary,
        ...
        >>> import rmmx
        >>> db = rmmx.DeviceBuffer(size=5)
        >>> db2 = rmmx.DeviceBuffer.to_device(b"abc")
...
cpdef DeviceBuffer to_device(const unsigned char[::1] b,
    ...
    --------
    >>> import rmmx
    >>> db = rmmx._lib.device_buffer.to_device(b"abc")
    >>> print(bytes(db))
    b'abc'
    """
...
cpdef void copy_ptr_to_host(uintptr_t db,
    ...
    >>> db = rmmx.DeviceBuffer.to_device(b"abc")
    >>> hb = bytearray(db.nbytes)
    >>> rmmx._lib.device_buffer.copy_ptr_to_host(db.ptr, hb)
    >>> print(hb)
    bytearray(b'abc')
    """
...
cpdef void copy_host_to_ptr(const unsigned char[::1] hb,
    ...
    >>> db = rmmx.DeviceBuffer(size=10)
    >>> hb = b"abc"
    >>> rmmx._lib.device_buffer.copy_host_to_ptr(hb, db.ptr)
    >>> hb = db.copy_to_host()
    >>> print(hb)
    array([97, 98, 99,  0,  0,  0,  0,  0,  0,  0], dtype=uint8)
    """
...
cpdef void copy_device_to_ptr(uintptr_t d_src,
    ...
    >>> db = rmmx.DeviceBuffer(size=5)
    >>> db2 = rmmx.DeviceBuffer.to_device(b"abc")
    >>> rmmx._lib.device_buffer.copy_device_to_ptr(db2.ptr, db.ptr, db2.size)
    >>> hb = db.copy_to_host()
    >>> hb
    array([97, 98, 99,  0,  0], dtype=uint8)
    """
