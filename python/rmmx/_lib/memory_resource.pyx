...
from rmmx._cuda.runtime import Error_t as cudaError_t

from rmmx._cuda.gpu import (
    CUDARuntimeError,
    driverGetVersion,
    getDevice,
    runtimeGetVersion,
...
from rmmx._lib.cuda_stream_view cimport cuda_stream_view
...
cdef class CallbackMemoryResource(DeviceMemoryResource):
    ...
    >>> dbuf = rmmx.DeviceBuffer(size=256)
...
def enable_logging(log_file_name=None):
    ...
    Use `rmmx.get_log_filenames()` to get the log file names
...
def disable_logging():
    ...
    `rmmx.initialize()` or `rmmx.enable_logging()`.
    """
    global _per_device_mrs
...
def get_log_filenames():
    ...
    --------
    >>> import rmmx
    >>> rmmx.reinitialize(devices=[0, 1], logging=True, log_file_name="rmmx.log")
    >>> rmmx.get_log_filenames()
    {0: '/home/user/workspace/rapids/rmm/python/rmmx.dev0.log',
     1: '/home/user/workspace/rapids/rmm/python/rmmx.dev1.log'}
    """
    global _per_device_mrs
