...
# cuda.set_memory_manager(rmmx.RMMNumbaManager)

_driver_version = rmmx._cuda.gpu.driverGetVersion()
_runtime_version = rmmx._cuda.gpu.runtimeGetVersion()
...
def test_rmm_modes(dtype, nelem, alloc, managed, pool):
    assert rmmx.is_initialized()
    array_tester(dtype, nelem, alloc)

    rmmx.reinitialize(pool_allocator=pool, managed_memory=managed)
...
def test_rmm_csv_log(dtype, nelem, alloc, tmpdir):
    ...
    rmmx.reinitialize(logging=True, log_file_name=base_name)
    array_tester(dtype, nelem, alloc)
    rmmx.mr._flush_logs()
...
def test_rmm_device_buffer(size):
    ...
    b2 = rmmx.DeviceBuffer.to_device(s)
    assert isinstance(b2, rmmx.DeviceBuffer)
    assert len(b2) == len(s)
...
def test_rmm_device_buffer_memoryview_roundtrip(hb):
    mv = memoryview(hb)
    db = rmmx.DeviceBuffer.to_device(hb)
    hb2 = db.copy_to_host()
...
def test_rmm_device_buffer_bytes_roundtrip(hb):
    try:
        ...
        with pytest.raises(TypeError):
            rmmx.DeviceBuffer.to_device(hb)
    else:
        if mv.format != "B":
...
def test_rmm_device_buffer_copy_from_host(hb):
    db = rmmx.DeviceBuffer.to_device(np.zeros(10, dtype="u1"))
...
@pytest.mark.parametrize(
    "cuda_ary",
    [
        lambda: rmmx.DeviceBuffer.to_device(b"abc"),
...
def test_rmm_pool_numba_stream(stream):
    ...
    stream = rmmx._cuda.stream.Stream(stream)
    a = rmmx._lib.device_buffer.DeviceBuffer(size=3, stream=stream)
...
def test_rmm_cupy_allocator():
    ...
    m = rmmx.rmm_cupy_allocator(42)
    assert m.mem.size == 42
    assert m.mem.ptr != 0
    assert isinstance(m.mem._owner, rmmx.DeviceBuffer)

    m = rmmx.rmm_cupy_allocator(0)
    assert m.mem.size == 0
    assert m.mem.ptr == 0
    assert isinstance(m.mem._owner, rmmx.DeviceBuffer)
...
def test_rmm_pool_cupy_allocator_with_stream(stream):
    ...
    # before rmmx.reinitialize(), otherwise it may segfault.
    del a
...
def test_pool_memory_resource(dtype, nelem, alloc):
    mr = rmmx.mr.PoolMemoryResource(
        rmmx.mr.CudaMemoryResource(),
        initial_pool_size=1 << 22,
        maximum_pool_size=1 << 23,
    )
    rmmx.mr.set_current_device_resource(mr)
...
#     "upstream",
#     [
#         lambda: rmmx.mr.CudaMemoryResource(),
#         lambda: rmmx.mr.ManagedMemoryResource(),
#     ],
# )
def test_fixed_size_memory_resource(dtype, nelem, alloc, upstream):
    mr = rmmx.mr.FixedSizeMemoryResource(
...
#         lambda: rmmx.mr.CudaMemoryResource(),
#         lambda: rmmx.mr.ManagedMemoryResource(),
#         lambda: rmmx.mr.PoolMemoryResource(
#             rmmx.mr.CudaMemoryResource(), 1 << 20
#         ),
#     ],
# )
def test_binning_memory_resource(dtype, nelem, alloc, upstream_mr):
    ...
    mr = rmmx.mr.BinningMemoryResource(upstream, 18, 22)

    # Test adding some explicit bin MRs
    fixed_mr = rmmx.mr.FixedSizeMemoryResource(upstream, 1 << 10)
    cuda_mr = rmmx.mr.CudaMemoryResource()
...
def test_reinitialize_max_pool_size():
    ...
    rmmx.DeviceBuffer().resize((1 << 23) - 1)
...
#     rmmx.enable_logging(log_file_name=base_name)
#     print(rmmx.mr.get_per_device_resource(0))
#     array_tester(dtype, nelem, alloc)
#     rmmx.mr._flush_logs()
...
def test_mr_devicebuffer_lifetime():
    ...
    rmmx.mr.set_current_device_resource(
        rmmx.mr.PoolMemoryResource(rmmx.mr.get_current_device_resource())
    )

    # Creates a new non-default stream
    stream = rmmx._cuda.stream.Stream()

    # Allocate DeviceBuffer with Pool and Stream
    a = rmmx.DeviceBuffer(size=10, stream=stream)

    # Change current MR. Will cause Pool to go out of scope
    rmmx.mr.set_current_device_resource(rmmx.mr.CudaMemoryResource())
...
def test_mr_upstream_lifetime():
    ...
    pool_mr = rmmx.mr.PoolMemoryResource(cuda_mr)
...
#     mr = rmmx.mr.CudaAsyncMemoryResource()
#     rmmx.mr.set_current_device_resource(mr)
#     assert rmmx.mr.get_current_device_resource_type() is type(mr)
#     array_tester(dtype, nelem, alloc)
...
#     try:
#         mr = rmmx.mr.CudaAsyncMemoryResource(enable_ipc=True)
...
def test_cuda_async_memory_resource_stream(nelems):
    ...
    dbuf = rmmx.DeviceBuffer.to_device(expected, stream=stream)
...
def test_statistics_resource_adaptor():
    ...
    mr = rmmx.mr.StatisticsResourceAdaptor(cuda_mr)

    rmmx.mr.set_current_device_resource(mr)

    buffers = [rmmx.DeviceBuffer(size=1000) for _ in range(10)]
    ...
    mr2 = rmmx.mr.StatisticsResourceAdaptor(mr)
    rmmx.mr.set_current_device_resource(mr2)

    for _ in range(2):
        buffers.append(rmmx.DeviceBuffer(size=1000))
...
def test_tracking_resource_adaptor():
    ...
    mr = rmmx.mr.TrackingResourceAdaptor(cuda_mr, capture_stacks=True)
    ...
    # Push a new Tracking adaptor
    mr2 = rmmx.mr.TrackingResourceAdaptor(mr, capture_stacks=True)
    rmmx.mr.set_current_device_resource(mr2)
...
def test_failure_callback_resource_adaptor():
    ...
    cuda_mr = rmmx.mr.CudaMemoryResource()
    mr = rmmx.mr.FailureCallbackResourceAdaptor(cuda_mr, callback)
    rmmx.mr.set_current_device_resource(mr)

    with pytest.raises(MemoryError):
        rmmx.DeviceBuffer(size=int(1e11))
    assert retried[0]
...
def test_dev_buf_circle_ref_dealloc():
    ...
    dbuf1 = rmmx.DeviceBuffer(size=1_000_000)
...
def test_mr_allocate_deallocate():
    mr = rmmx.mr.TrackingResourceAdaptor(rmmx.mr.get_current_device_resource())
    size = 1 << 23  # 8 MiB
...
def test_custom_mr(capsys):
    base_mr = rmmx.mr.CudaMemoryResource()
    ...
    rmmx.mr.set_current_device_resource(
        rmmx.mr.CallbackMemoryResource(allocate_func, deallocate_func)
    )

    rmmx.DeviceBuffer(size=256)
...
def make_reinit_hook():
    ...
    def _make_reinit_hook(func, *args, **kwargs):
        funcs.append(func)
        rmmx.register_reinitialize_hook(func, *args, **kwargs)
        return func

    yield _make_reinit_hook
    for func in funcs:
        rmmx.unregister_reinitialize_hook(func)
...
def test_reinit_hooks_unregister(make_reinit_hook):
    ...
    rmmx.unregister_reinitialize_hook(one)
    rmmx.reinitialize()
    assert L == [2]
...
def test_reinit_hooks_unregister_twice_registered(make_reinit_hook):
    ...
    rmmx.unregister_reinitialize_hook(func_with_arg)
    rmmx.reinitialize()
    assert L == [2]
...
#     cuda_ary = cuda_ary()
#     db = rmmx.DeviceBuffer.to_device(np.zeros(5, dtype="u1"))
#     db.copy_from_device(cuda_ary)
